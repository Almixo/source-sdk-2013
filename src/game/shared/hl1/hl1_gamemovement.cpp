...
bool CHL1GameMovement::CheckJumpButton( void )
{
	...
	if ( (  m_pHL1Player->m_Local.m_bDucking ) || (  m_pHL1Player->GetFlags() & FL_DUCKING ) )
	{
			...
			mv->m_vecVelocity.Length() > 50 )
		{
			...
			mv->m_vecVelocity.z = sqrt(2 * 800 * 70.0);
		}
		else
		{
			mv->m_vecVelocity[2] = flGroundFactor * sqrt(2 * 800 * 56.0);  // 2 * gravity * height
		}
	}
	else
	{
...
bool CHL1GameMovement::CanUnduck()
{
	int i;
	trace_t trace;
	Vector vecOrigin, newOrigin;

	vecOrigin = mv->GetAbsOrigin();

	VectorCopy( vecOrigin, newOrigin );
	...
	TracePlayerBBox( vecOrigin, newOrigin, MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, trace );
...
void CHL1GameMovement::FinishUnDuck( void )
{
	...
	VectorCopy( newOrigin, vecOrigin );
