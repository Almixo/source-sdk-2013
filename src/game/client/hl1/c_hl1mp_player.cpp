...
void C_HL1MP_Player::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	if ( m_takedamage != DAMAGE_NO )
	{
		...
		if ( pAttacker )
		{
			if ( HL1MPRules()->IsTeamplay() && pAttacker->InSameTeam( this ) == true )
				return;
		}
